...
function parseWeek(rows) {
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    if (DAY_MAP[c0] !== undefined) {
      curDay = DAY_MAP[c0];
      if (!plan[curDay])
        plan[curDay] = { manha: [], tarde: [], pool: c1 || "P25" };
    }
    ...
    if (
      BLOCK_NAMES.has(mZona) ||
      (mZona.startsWith("TAREFA") && !ZONA_SET.has(mZona))
    ) {
      const tipo = mZona.includes("AQUEC")
        ? "aquec"
        : mZona.includes("RECUP")
          ? "rec"
          : "tarefa";
      plan[curDay].manha.push({
        isHeader: true,
        tipo,
        nome: String(row[COL.M_zona] || "").trim(),
        metros: 0,
        zona: "",
        desc: "",
        ciclo: "",
      ...
      if (!zonePlan[key])
        zonePlan[key] = Object.fromEntries(ZONAS.map((z) => [z, 0]));
    ...
    if (
      BLOCK_NAMES.has(tZona) ||
      (tZona.startsWith("TAREFA") && !ZONA_SET.has(tZona))
    ) {
      const tipo = tZona.includes("AQUEC")
        ? "aquec"
        : tZona.includes("RECUP")
          ? "rec"
          : "tarefa";
      plan[curDay].tarde.push({
        isHeader: true,
        tipo,
        nome: String(row[COL.T_zona] || "").trim(),
        metros: 0,
        zona: "",
        desc: "",
        ciclo: "",
...
function loadXlsx(e) {
  ...
  if (hasPlan) {
    get("importModalFile").textContent = file.name;
    get("importModal").classList.add("open");
...
function readXlsx(file, mode) {
  ...
  msg.textContent = "A ler " + file.name + "…";
  ...
  reader.onload = (ev) => {
    try {
      ...
      const exCount = Object.values(plan).reduce(
        (a, d) =>
          a +
          d.manha.filter((b) => !b.isHeader).length +
          d.tarde.filter((b) => !b.isHeader).length,
        0,
      );
      ...
      renderDayStrip();
      renderPlan();
      if (document.getElementById("screen-zonas").classList.contains("active"))
        renderZones();
...
function openAddBlock() {
  get("addBlockModal").classList.add("open");
}
function closeAddBlock() {
  get("addBlockModal").classList.remove("open");
}
...
function saveBlock() {
  ...
  if (!S.weekPlan[S.activeDayIdx])
    S.weekPlan[S.activeDayIdx] = { manha: [], tarde: [], pool: "P25" };
  ...
  if (zona && metros) {
    ...
    if (!S.zonePlan[key])
      S.zonePlan[key] = Object.fromEntries(ZONAS.map((z) => [z, 0]));
    S.zonePlan[key][zona] = (S.zonePlan[key][zona] || 0) + metros;
  }
  ["bNome", "bDesc", "bMetros", "bCiclo", "bTarget"].forEach(
    (id) => (get(id).value = ""),
  );
  closeAddBlock();
  save();
...
// ---- ATHLETES ----
function addAthlete() {
  const nome = get("nomeAtleta").value.trim();
  if (!nome) return;
  S.athletes.push({
    id: Date.now(),
    nome,
    grupo: get("grupoAtleta").value.trim(),
  });
  get("nomeAtleta").value = "";
  get("grupoAtleta").value = "";
  save();
  renderAthletes();
...
function renderAthletes() {
  ...
  if (!S.athletes.length) {
    el.innerHTML = `<div class="empty"><div class="icon">👤</div>Sem atletas. Adicione acima.</div>`;
    return;
  }
  el.innerHTML = S.athletes
    .map(
      (a) => `
...
function populateCrono() {
  ...
  get("cBloco").innerHTML = headers.length
    ? headers
        .map(
          (b, i) =>
            `<option value="${i}">${b.nome || "Bloco " + (i + 1)}</option>`,
        )
        .join("")
...
function toggleSW() {
  ...
  if (sw.running) {
    ...
    sw.running = true;
    _swInt = setInterval(() => {
      get("swDisp").textContent = fmtTime(sw.elapsed + (Date.now() - sw.start));
    }, 37);
...
function takeSplit() {
  const sw = S.sw,
    now = sw.elapsed + (sw.running ? Date.now() - sw.start : 0);
  sw.splits.push({ cum: now, lap: now - sw.lastSplit });
  sw.lastSplit = now;
  renderSplits();
  get("swInfo").textContent =
    `${sw.splits.length} parcial${sw.splits.length !== 1 ? "is" : ""}`;
}
...
function parseTargetTime(s) {
  ...
  if (m)
    return (
      (parseInt(m[1]) * 60 + parseInt(m[2])) * 1000 + parseInt(m[3] || 0) * 100
    );
...
function renderSplits() {
  const el = get("splitList"),
    splits = S.sw.splits;
  if (!splits.length) {
    el.innerHTML = `<div style="font-size:12px;color:var(--text-3);padding:10px 0;text-align:center;">Sem parciais ainda</div>`;
    return;
  }
  const avg = splits.reduce((a, s) => a + s.lap, 0) / splits.length;
  const headers =
    (S.weekPlan[S.activeDayIdx] || {})[S.sessao]?.filter((b) => b.isHeader) ||
    [];
  ...
  el.innerHTML = `<table class="split-table">
    ...
    <tbody>${splits
      .map((s, i) => {
        const d = s.lap - avg,
          ds = (d >= 0 ? "+" : "") + fmtTime(Math.abs(d));
        const isBest = s.lap === best && splits.length > 1;
        let tc = "";
        if (tMs) {
          const td2 = s.lap - tMs;
          tc = `<td class="${td2 > 0 ? "dp" : "dn"}">${(td2 >= 0 ? "+" : "") + fmtTime(Math.abs(td2))}</td>`;
        }
...
function resetSW() {
  ...
  get("swDisp").textContent = "00:00.00";
  get("swInfo").textContent = "Sem parciais";
  get("btnSS").textContent = "Iniciar";
  get("btnSS").className = "btn primary lg";
  get("btnParcial").disabled = true;
  renderSplits();
}

function saveSplits() {
  if (!S.sw.splits.length) {
    toast("Sem parciais para guardar.");
    return;
  }
  ...
  S.results.push({
    ...
    dia: DIAS_PT[S.activeDayIdx],
    diaIdx: S.activeDayIdx,
    sessao: S.sessao,
    athlete: athlete.nome,
    bloco: bloco?.nome || "—",
    targetTime: bloco?.target || "",
    splits: [...S.sw.splits],
  });
  save();
  resetSW();
  toast("Parciais guardados para " + athlete.nome + "! ✓");
}

// ---- ZONES ----
function registerZone() {
  const zona = get("zZona").value,
    metros = parseInt(get("zMetros").value) || 0;
  if (!metros) {
    toast("Introduza os metros realizados.");
    return;
  }
  const key = `${S.activeDayIdx}_${S.sessao}`;
  if (!S.zoneLog[key])
    S.zoneLog[key] = Object.fromEntries(ZONAS.map((z) => [z, 0]));
...
function renderZones() {
  ...
  get("zSessaoLabel").textContent =
    `${DIAS_PT[S.activeDayIdx]} — ${S.sessao === "manha" ? "Manhã" : "Tarde"}`;

  const rows = ZONAS.map((z) => {
    const p = plan[z] || 0,
      l = log[z] || 0;
    if (!p && !l) return "";
    const barW = p ? Math.min(100, Math.round((l / p) * 100)) : 0,
      over = l > p && p > 0;
    return `<tr>
...
function renderResults() {
  const fa = get("rFiltAtleta").value,
    fd = get("rFiltDia").value;
  const athletes = [...new Set(S.results.map((r) => r.athlete))].sort();
  const dias = [...new Set(S.results.map((r) => r.dia))];
  get("rFiltAtleta").innerHTML =
    '<option value="">Todos os atletas</option>' +
    athletes
      .map((a) => `<option${fa === a ? " selected" : ""}>${a}</option>`)
      .join("");
  get("rFiltDia").innerHTML =
    '<option value="">Todos os dias</option>' +
    dias
      .map((d) => `<option${fd === d ? " selected" : ""}>${d}</option>`)
      .join("");
  ...
  if (!data.length) {
    el.innerHTML = `<div class="empty"><div class="icon">📋</div>Sem resultados guardados ainda.</div>`;
    return;
  }
  ...
  el.innerHTML = data
    ...
    .map((r) => {
      const avg =
        r.splits.reduce((a, s) => a + s.lap, 0) / (r.splits.length || 1);
      ...
      <div class="result-body"><table class="split-table">
        ...
        <tbody>${r.splits
          .map((s, i) => {
            const d = s.lap - avg,
              isBest = s.lap === best && r.splits.length > 1;
            let tc = "";
            if (tMs) {
              const td2 = s.lap - tMs;
              tc = `<td class="${td2 > 0 ? "dp" : "dn"}">${(td2 >= 0 ? "+" : "") + fmtTime(Math.abs(td2))}</td>`;
...
function exportCSV() {
  if (!S.results.length) {
    toast("Sem resultados para exportar.");
    return;
  }
  const rows = [
    [
...
document.addEventListener("DOMContentLoaded", () => {
  get("topbarDate").textContent = new Date().toLocaleDateString("pt-PT", {
    weekday: "long",
    day: "numeric",
    month: "long",
    year: "numeric",
